...
//  Shared
//
//  Created by Aidan Lewis-Grenz and Dean Nunis
//  Reasurch by
//
//  Panther Buddy App
// Who are the users?
...
struct ContentView: View {
    ...
    //words of affermation array: Dean
    ...
    //user information: Aidan
    @State public var person = "Aidan"
    @State public var annonymous = false
    @State public var username = "aidanlewisgrenz"
    @State public var pantherEmail = "lewisgrenz@chapman.edu"
    @State private var nickName = "ALG"
    @State private var birthday = "02/12/2002"
    @State private var pass = "Monkey2282"
    @State private var struggle = "Anxiety"
    
    //chat message user types: Aidan
    @State public var messageToSend = ""
    
    //list of items in Home list: Aidan
    @State private var users = [
            User(user: "Aidan", annonymous: false),
            User(user: "Kevin", annonymous: false),
            User(user: "Shelby", annonymous: false)
    ]
    
    //list of items in ToDo list: Dean
    ...
    //list of Items in Let's Talk Tab: Aidan
    @State public var chats = [
        Chat(person:"Miles", conversations: ["Aidan: Hello there!", "Annonymous: Hi aidan"]),
        Chat(person:"Annonymous", conversations: ["Aidan: Hello there!", "Annonymous: Hi aidan"]),
        Chat(person:"Kevin", conversations: ["Aidan: Hello there!", "Annonymous: Hi aidan"]),
        Chat(person:"Fay", conversations: ["Aidan: Hello there!", "Annonymous: Hi aidan"]),
        Chat(person:"Annonymous", conversations: ["Aidan: Hello there!", "Annonymous: Hi aidan"])
        ]
    ...
    TabView(selection: $selection) {
        ...
        //tab view "Home Page" : Dean and Aidan
        NavigationView{
            //list of items : Dean and Aidan
            List{
                Text("Welcome \(person)")
        ...
        //tab view "Let's Talk": Dean and Aidan
        NavigationView{
            //list of items: Dean and Aidan
            List{
                //Existing conversations: Aidan
                NavigationLink(destination:
                    VStack{
                        List{
                ...
                //findiong someone anonymous : Aidan and Dean
                NavigationLink(destination:
                    VStack{
        ...
        //tab four: Dean and Aidan
        NavigationView{
            //list of items
            List{
                //privacy policy: Dean and Aidan + GCI team
                NavigationLink(destination:
                    VStack{
                        Text("Privacy Policy")
                        .foregroundColor(colors[colorIndex])
                            .bold()
                        Spacer()
                        Text("https://docs.google.com/document/d/1Pqim2w6tNRb_ISKDZHgydHpIWAUkEAHVch9bi8HEdrM/edit?usp=sharing")
                        .foregroundColor(colors[colorIndex])
                        Spacer()
                    }
                ){
                ...
                //contact chapman: Dean and Aidan
                NavigationLink(destination:
                    VStack{
    ...
    //stop 5min meditation timers: Dean
    func stopFiveTimer(){
        timerFivePaused = true
        timerFive?.invalidate()
        timerFive = nil
    }
    //stop 10min meditation timers: Dean
    func stopTenTimer(){
        timerTenPaused = true
        timerTen?.invalidate()
        timerTen = nil
    }
    //stop 20min  meditation timers: Dean
    ...
    //reset 5min meditation timers: Dean
    func restartFiveTimer(){
        minutes5 = 5
        secondsFive = 0
    }
    //reset 10min meditation timers: Dean
    func restartTenTimer(){
        minutes10 = 10
        secondsTen = 0
    }
    //reset 20min meditation timers: Dean
    func restartTwentyTimer(){
        minutes20 = 20
        secondsTwenty = 0
    }
}
//for Todo wiring: Aidan
struct Todo: Identifiable{
    let id = UUID()
    let name: String
    let discription: String
}
//for Chat wiring: Aidan
struct Chat {
    var person: String
    var conversations: [String] = []
}
//for user wiring: Aidan
struct User {
    let user: String
    let annonymous: Bool
}
//for adding todos: Dean
